...
def findSongScore(currentSong):
    ...
    #get the score csv
def loadSongScore(scoreFile, score, initials):
...
def loadSurvScore(score, initials):
...
#function to find music database based on song title
def findDBFile(songName):
    #open songs.csv
    songDir = pd.read_csv('code/assets/databases/songs.csv')
    #find the song by its name
    #find the music file in that row
    #return that music file's name
    return(songDir[songDir['songTitle'] == songName]['scoreFile'].values[0])
...
#function to find song title based on music database
def findTitlefromDB(songFile):
    #open songs.csv
    songDir = pd.read_csv('code/assets/databases/songs.csv')
    #find the song by its music file
    #find the name in that row
    #return the song title associated with the music file
    return(songDir[songDir['scoreFile'] == songFile]['songTitle'].values[0])
